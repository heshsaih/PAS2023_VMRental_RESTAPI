...
public class userEndpointTest {
    ...
    void getUserByIdTest() throws Exception {
        ...
        mockMvc.perform(get("/users/{id}",id).header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/users/{id}","123").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
    ...
    void getByUsernameContainsTest() throws Exception {
        ...
        setup();
        var users = userManager.findAllUsers();
        mockMvc.perform(get("/users/getbyusernamecontains/na").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
    ...
    void getByUsername() throws Exception{
        ...
        mockMvc.perform(get("/users/getbyusername").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag)
    ...
    void createUserTest() throws Exception {
        ...
        mockMvc.perform(post("/users/createclient").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag)
                .content(json)
                .contentType("application/json"))
                .andExpect(status().isOk())
        ...
        userAdministrator.setPassword("test2");
        String json2 = objectMapper.writeValueAsString(userAdministrator);
        mockMvc.perform(post("/users/createadministrator").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag)
                .content(json2)
                .contentType("application/json"))
                .andExpect(status().isOk())
        ...
        userResourceManager.setPassword("test2");
        String json3 = objectMapper.writeValueAsString(userResourceManager);
        mockMvc.perform(post("/users/createresourcemanager").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag)
                .content(json3)
                .contentType("application/json"))
                .andExpect(status().isBadRequest());
    ...
    void updateUserTest() throws Exception {
        ...
        String json = objectMapper.writeValueAsString(userClient);
        mockMvc.perform(put("/users/{id}",client.getId()).header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag)
                .content(json)
                .contentType("application/json"))
                .andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.address.street").value(client.getAddress().getStreet()))
                .andExpect(jsonPath("$.address.houseNumber").value(client.getAddress().getHouseNumber()));
        mockMvc.perform(put("/users/{id}","123").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag)
                .content(json)
                .contentType("application/json"))
                .andExpect(status().isNotFound());
    ...
    void activateAndDeactivateUserTest() throws Exception {
        ...
        mockMvc.perform(patch("/users/{id}/deactivate",client.getId()).header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
                .andExpect(status().isOk());
        client = (Client) userManager.findAllUsers().get(0);
        Assertions.assertFalse(client.isActive());
        mockMvc.perform(patch("/users/{id}/activate",client.getId()).header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
                .andExpect(status().isOk());
        client = (Client) userManager.findAllUsers().get(0);
        Assertions.assertTrue(client.isActive());
        mockMvc.perform(patch("/users/{id}/deactivate","123").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
                .andExpect(status().isNotFound());
        mockMvc.perform(patch("/users/{id}/activate","123").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
                        .header(HttpHeaders.IF_MATCH, etag))
                .andExpect(status().isNotFound());
    }
